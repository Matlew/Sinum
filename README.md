...
-- [PRINT] "Správa pater"
87 General: Basic library extensions
...
Arguments:
• msw (number) — most significant word
88 General: Basic library extensions
• lsw (number) — least significant word
Execution context
context()
...
105 Utilities: Color operations
Arguments:
...
Returns:
106 Utilities: Color operations
...
107 Utilities: Color operations
Arguments:
...
Arguments:
• url (string) — a valid URL
136 Utilities: URL manipulation
Example:
...
Example:
137 Utilities: URL manipulation
local port
...
138 Utilities: URL manipulation
Returns:
• stripped url (string)
Arguments:
• url (string) — a valid URL
...
MULTIPLE_WRITE )
164 Network: Modbus client (master)
...
165 Network: Modbus client (master)
...
166 Network: Modbus client (master)
...
◦ (userdata) Modbus response reference, for chained calls
Arguments:
170 Network: Modbus server (slave)
...
for i = 1, size do
data[i] = true
171 Network: Modbus server (slave)
end
...
172 Network: Modbus server (slave)
...
178 Network: SMTP
SMTP
Global scope object which allows user to send e-mails using configured SMTP server.
Available in all contexts. You can access it using smtp object.
Methods
• settings()
Returns settings object which can be used to configure SMTP server connection.
Returns: Settings
• plainEmail()
Creates a mail info object which can be configured and used to send email. Assumes
email body is plain text.
NOTE: You can only send 6 e-mails an hour
Returns: MailInfo
• htmlEmail()
Creates a mail info object which can be configured and used to send email. Assumes
email body is HTML text.
NOTE: You can only send 6 e-mails an hour
Returns: MailInfo
• onEmailSent()
Callback hook. Calls function passed in argument when e-mail sending finishes.
Arguments:
◦ function (function, required) — callback function which should be called
Arguments:
• success (boolean) — indicates if e-mail sending process succeeded
• recipient (string) — first e-mail recipient to connect callback call with e-mail
• error (string) — error message in case of failure, empty string otherwise
Settings Methods
Object which can be used to configure SMTP server information and credentials.
• getValue(property_name)
Returns value of object property.
Returns:
◦ any - depends on property type
...
Sets value for object property.
Returns:
◦ userdata — reference to settings object, for call chains
Arguments:
◦ property_name (string) — name of property
◦ property_value (any) — property type dependant value which should be set
Properties
...
SMTP server address
• port (integer)
Port number used to connect to SMTP server
• secure (boolean)
Indicates if connection with SMTP server is secure (if it is using TLS/SSL)
• username (string)
Username used to log in to SMTP server. If empty log-in is omitted.
• password (string)
Password used to log in to SMTP server.
NOTE: It is not readable. "********" is returned when accesed by getValue .
MailInfo Methods
These methods allow customizing the e-mail and send it.
• addRecipient(recipient)
Adds primary e-mail recipient.
...
Adds CC (carbon copy) e-mail recipient.
NOTE: can be called multiple times.
Arguments:
◦ recipient (string) — e-mail address of CC recipient
Returns: MailInfo
• addBccRecipient(recipient)
Adds BCC (blind carbon copy) e-mail recipient.
NOTE: can be called multiple times.
Arguments:
◦ recipient (string) — e-mail address of BCC recipient
Returns: MailInfo
• subject(subject)
Sets e-mail subject.
Arguments:
...
Returns: MailInfo
• send()
Send the e-mail. Fails when recipients are not configured.
Examples
Send plain text e-mail to single recipient
smtp:plainEmail()
:addRecipient("john@example.com")
:subject("Hello")
:body("I'm sending you an e-mail from my Sinum Central.")
:send()
181 Network: SMTP
Send HTML text e-mail to two BCC recipients (they won’t know about each
other)
smtp:htmlEmail()
:addBccRecipient("alice@example.com")
:addBccRecipient("susan@example.com")
:subject("My love")
:body([[<html> <font color="red">I love you!</font></html>]])
:send()
182 Network: TCP port knocking
TCP port knocking
...
186 Network: Websocket client
Websocket client
Global scope objects which allow user to exchange Websocket messages.
...
Attempting to reference a nonexistent client or set the wrong value type will result in a
script error.
The client name , url , headers and query_params should be configured by REST (but
can be changed later in the script).
Properties
Properties direct access is not allowed. You can get or set values using setValue ,
getValue methods.
Attempting to reference a nonexistent object, retrieve a nonexistent object property, or set
...
190 Energy center: Energy consumption
Energy consumption
...
191 Energy center: Energy consumption
Available Properties
...
◦ property_name (string) — name of property
◦ property_value (any) — property type dependant value which should be set
193 Energy center: Energy prices
...
194 Energy center: Energy prices
Return:
...
195 Energy center: Energy prices
Returns:
◦ userdata - reference to Energy Prices object, for call chains
Arguments:
...
196 Energy center: Energy prices
Returns:
◦ boolean
Arguments:
◦ hour (integer) — hour in range 0–23
...
◦ number[] — table of prices with 24–96 elements (for each time interval of the day)
Arguments:
197 Energy center: Energy prices
◦ accessType (string?) — allows selecting data source. One of: "api" , "lua" . Falls
back to configured access_type property if not provided.
• clearNextDayPrices()
...
◦ minutes (integer) — minutes, a multiple of prices_interval , less than 60.
198 Energy center: Energy prices
◦ price (number) — energy price to set
...
199 Energy center: Energy prices
Note
...
200 Energy center: Energy production
Energy production
...
201 Energy center: Energy production
...
Unit: 1 Wh
202 Energy center: Energy storage
Energy storage
...
Available Properties
• available (boolean, read-only)
203 Energy center: Energy storage
...
204 Energy center: Flow monitor
Flow monitor
The goal of this module is to provide an easy to understand and visualize way of displaying
...
Direct access to properties is not allowed. You can read values using the getValue
method. An attempt at retrieving a nonexistent object property will cause a script error.
205 Energy center: Flow monitor
Available Properties
...
are possible.
206 Energy center: Flow monitor
Unit: 1 mW
...
207 Energy center: Flow monitor
...
212 Schedules: Temperature curve
Temperature curve
...
instantly.
213 Schedules: Temperature curve
Unit: 0.1 °C
...
Commands
There are no commands available for this type of schedule.
214 Schedules: Temperature curve
Examples
...
physical device.
Value range: ⟨0; 10⟩
221 WTP devices: Air quality sensor
Air quality sensor
...
111 – 150 unhealthy
> 150 very_unhealthy
222 WTP devices: Air quality sensor
...
• software_version (string, read-only)
• sub_id (integer, read-only)
223 WTP devices: Blind controller
Blind controller
...
224 WTP devices: Blind controller
...
225 WTP devices: Blind controller
...
226 WTP devices: Blind controller
...
227 WTP devices: Blind controller
• variant (string, read-only) = "generic"
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
• purpose (string)
...
228 WTP devices: Blind controller
• tilt
...
250 WTP devices: Humidity sensor
Humidity sensor
...
• purpose (string)
• supervised_properties (array, read-only)
251 WTP devices: Humidity sensor
• delayed_actions (array, read-only)
WTP device properties (full spec)
• address (integer, read-only)
• battery (integer, read-only)
...
260 WTP devices: Opening sensor
Opening sensor
...
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
261 WTP devices: Opening sensor
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
WTP device properties (full spec)
...
262 WTP devices: Pressure sensor
Pressure sensor
...
• variant (string, read-only) = "generic"
• visible (boolean, read-only)
263 WTP devices: Pressure sensor
• voice_assistant_device_type (string, read-only)
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
...
• software_version (string, read-only)
• sub_id (integer, read-only)
264 WTP devices: RGB controller
RGB controller
...
265 WTP devices: RGB controller
Device properties (full spec)
• class (string, read-only) = "wtp"
• color (string)
• icon (string)
...
266 WTP devices: RGB controller
• turn_off
Turns off output.
Argument
Optional: transition time in 0.1s (default 5 - 500ms) (number)
...
• minimum: 1 (100 ms)
• maximum: 6000 (10 minutes)
267 WTP devices: RGB controller
• parameter is optional (500 ms default)
• activate_animation
...
268 WTP devices: RGB controller
...
269 WTP devices: Radiator actuator
Radiator actuator
...
Device properties (full spec)
• class (string, read-only) = "wtp"
270 WTP devices: Radiator actuator
• color (string)
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
...
271 WTP devices: Radiator actuator
Note
...
279 WTP devices: Temperature regulator
...
280 WTP devices: Temperature regulator
...
• class (string, read-only) = "wtp"
• color (string)
281 WTP devices: Temperature regulator
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
...
constant .
282 WTP devices: Temperature regulator
Argument:
...
283 WTP devices: Temperature sensor
Temperature sensor
...
284 WTP devices: Temperature sensor
• variant (string, read-only) = "generic"
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
• purpose (string)
...
288 WTP devices: Two-state input sensor
...
• purpose (string)
• supervised_properties (array, read-only)
289 WTP devices: Two-state input sensor
• delayed_actions (array, read-only)
WTP device properties (full spec)
• address (integer, read-only)
• battery (integer, read-only)
...
295 TECH RS devices: Common heat buffer
Common heat buffer
...
296 TECH RS devices: Common heat buffer
...
• address (string, read-only)
297 TECH RS devices: Additional CH pump
Additional CH pump
...
• messages (sequence, read-only)
• name (string)
298 TECH RS devices: Additional CH pump
• room_id (integer, read-only, optional)
• status (string, read-only)
• tags (string[])
...
TECH device properties (full spec)
• address (string, read-only)
299 TECH RS devices: Common DHW
Common DHW
...
Unit: 0.1 °C
300 TECH RS devices: Common DHW
• pump_work (boolean, read-only)
Current pump working state on/off.
Device properties (full spec)
• class (string, read-only) = "tech"
...
301 TECH RS devices: Common DHW
dhw:setValue("target_temperature", 55)
end
end
302 TECH RS devices: Additional DHW pump
Additional DHW pump
...
• icon (string)
• id (integer, read-only)
303 TECH RS devices: Additional DHW pump
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
• room_id (integer, read-only, optional)
...
TECH device properties (full spec)
• address (string, read-only)
304 TECH RS devices: Additional floor pump
...
• labels (string[], read-only)
• messages (sequence, read-only)
305 TECH RS devices: Additional floor pump
• name (string)
• room_id (integer, read-only, optional)
• status (string, read-only)
• tags (string[])
...
309 TECH RS devices: Humidity sensor
Humidity sensor
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
310 TECH RS devices: Humidity sensor
TECH device properties (full spec)
• address (string, read-only)
311 TECH RS devices: Pellet boiler
Pellet boiler
...
Current return temperature.
Unit: 0.1 °C
312 TECH RS devices: Pellet boiler
...
313 TECH RS devices: Pellet boiler
Device properties (full spec)
• class (string, read-only) = "tech"
• color (string)
• icon (string)
...
314 TECH RS devices: Main pellet CH
Main pellet CH
...
• voice_assistant_device_type (string, read-only)
• purpose (string)
315 TECH RS devices: Main pellet CH
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
TECH device properties (full spec)
• address (string, read-only)
...
316 TECH RS devices: Additional protection pump
...
• room_id (integer, read-only, optional)
• status (string, read-only)
317 TECH RS devices: Additional protection pump
• tags (string[])
• type (string, read-only) = "pellet_ch_main"
• variant (string, read-only) = "generic"
• visible (boolean, read-only)
...
TECH device properties (full spec)
• address (string, read-only)
318 TECH RS devices: Additional relay
Additional relay
...
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
319 TECH RS devices: Additional relay
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
TECH device properties (full spec)
• address (string, read-only)
320 TECH RS devices: Relay
Relay
...
323 TECH RS devices: Solar
Solar
Device plugged into RS input in central unit. Solar representation. Allows user to read
and modify solar controller parameters.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
and setValue methods. An attempt at retrieving a nonexistent object property, or setting
wrong value type will cause a script error.
• solar_pump (boolean, read-only)
Solar pump working state on/off.
• secondary_pump (boolean, read-only)
Secondary pump working state on/off.
• peripheral_device (boolean, read-only)
Peripheral device working state on/off.
• temperature_collector_1 (number, read-only)
Current collector 1 temperature.
Unit: 0.1 °C
• temperature_collector_2 (number, read-only)
Current collector 2 temperature.
Unit: 0.1 °C
• temperature_tank (number, read-only)
Current tank temperature.
Unit: 0.1 °C
• temperature_additional (number, read-only)
Current additional temperature.
Unit: 0.1 °C
• scheme (number, read-only)
Solar installation scheme ID.
• work_mode (number)
Solar work mode ID.
Device properties (full spec)
• class (string, read-only) = "tech"
324 TECH RS devices: Solar
• color (string)
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
...
TECH device properties (full spec)
• address (string, read-only)
325 TECH RS devices: Temperature regulator
Temperature regulator
...
• target_temperature_maximum (number)
Upper limit of the target temperature. Could not be less than minimum. Setting
maximum below target, will also change target value to minimum. Unit: 0.1 °C
• target_temperature_reached (boolean)
326 TECH RS devices: Temperature regulator
...
Calls Temperature Regulator to change constant or time_limited mode target
temperature to the desired value.
327 TECH RS devices: Temperature regulator
If regulator works in time_limited mode it will change target temperature only, not
affecting remaining_time .
If regulator works in schedule mode it will change target temperature mode to
constant .
...
Note
Cannot be executed when regulator is not associated with a thermostat.
328 TECH RS devices: Temperature regulator
Examples
Raise target temperature between 15:00 and 20:00
if dateTime:changed() then
if dateTime:getHours() == 15 and dateTime:getMinutes() == 0 then
...
329 TECH RS devices: Temperature sensor
Temperature sensor
...
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
330 TECH RS devices: Temperature sensor
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
TECH device properties (full spec)
• address (string, read-only)
331 TECH RS devices: Two state input sensor
Two state input sensor
...
• purpose (string)
• supervised_properties (array, read-only)
332 TECH RS devices: Two state input sensor
• delayed_actions (array, read-only)
TECH device properties (full spec)
• address (string, read-only)
333 TECH RS devices: Valve
Valve
...
336 TECH RS devices: Ventilation
Ventilation
...
Current temperature entering the house Unit: 0.1 °C
337 TECH RS devices: Ventilation
...
338 TECH RS devices: Ventilation
...
Unit: 1 %
339 TECH RS devices: Ventilation
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
340 TECH RS devices: Ventilation
TECH device properties (full spec)
• address (string, read-only)
Commands
• cooling_on_request
...
342 Modbus devices: Alpha-Innotec — Heat pump
...
Unit: 0.1 °C.
343 Modbus devices: Alpha-Innotec — Heat pump
...
Unit: 0.1 °C.
344 Modbus devices: Alpha-Innotec — Heat pump
• heat_demand (boolean)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
345 Modbus devices: Alpha-Innotec — Main DHW
...
• labels (string[], read-only)
• messages (sequence, read-only)
346 Modbus devices: Alpha-Innotec — Main DHW
• name (string)
• room_id (integer, read-only, optional)
• status (string, read-only)
• tags (string[])
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
347 Modbus devices: Alpha-Innotec — Temperature sensor
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
348 Modbus devices: Ampowr Ampi Home 1 Phase — Inverter
...
349 Modbus devices: Ampowr Ampi Home 1 Phase — Inverter
Unit: 1 Wh (with accuracy of 10 Wh)
...
• id (integer, read-only)
• labels (string[], read-only)
350 Modbus devices: Ampowr Ampi Home 1 Phase — Inverter
• messages (sequence, read-only)
• name (string)
• room_id (integer, read-only, optional)
• status (string, read-only)
...
Active power limit.
Unit: 1 % (number)
351 Modbus devices: Ampowr Ampi Home 1 Phase — Battery
...
352 Modbus devices: Ampowr Ampi Home 1 Phase — Battery
...
Unit: 1 %
353 Modbus devices: Ampowr Ampi Home 1 Phase — Battery
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
354 Modbus devices: Ampowr Ampi Home 1 Phase — Battery
Commands
• charge
...
Can only be changed when inverter_working_mode is peak_shift . Device has
label battery_charge_support when available.
355 Modbus devices: Ampowr Ampi Home 1 Phase — Energy meter
...
Unit: 1 Wh (with accuracy of 10 Wh)
356 Modbus devices: Ampowr Ampi Home 1 Phase — Energy meter
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
357 Modbus devices: Ampowr Ampi Home 3 Phase — Inverter
...
• energy_produced_month (number, read-only)
Amount of energy produced by photovoltaic panels this month.
358 Modbus devices: Ampowr Ampi Home 3 Phase — Inverter
Unit: 1 Wh (with accuracy of 10 Wh)
• energy_produced_year (number, read-only)
Amount of energy produced by photovoltaic panels this year.
Unit: 1 Wh (with accuracy of 10 Wh)
...
• id (integer, read-only)
• labels (string[], read-only)
359 Modbus devices: Ampowr Ampi Home 3 Phase — Inverter
• messages (sequence, read-only)
• name (string)
• room_id (integer, read-only, optional)
• status (string, read-only)
...
Active power limit.
Unit: 1 % (number)
360 Modbus devices: Ampowr Ampi Home 3 Phase — Battery
...
Unit: 1 %
Range: 5 % – 90 %
361 Modbus devices: Ampowr Ampi Home 3 Phase — Battery
• on_grid_depth_of_discharge (number)
Sets on grid depth of discharge. Can only be changed when
depth_of_discharge_enabled and depth_of_discharge_bms_enabled are set to
true .
...
Limiting charging power.
Unit: 1 %
362 Modbus devices: Ampowr Ampi Home 3 Phase — Battery
• grid_max_soc_charge (number)
Limiting charging battery from grid.
Unit: 1 %
Range: 20 % – 100 %
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
363 Modbus devices: Ampowr Ampi Home 3 Phase — Battery
Commands
• charge
Sends request to force charge the battery.
Note
...
Can only be changed when inverter_working_mode is peak_shift . Device has
label battery_charge_support when available.
364 Modbus devices: Ampowr Ampi Home 3 Phase — Energy meter
...
Amount of energy fed to the power grid this year.
Unit: 1 Wh (with accuracy of 10 Wh)
365 Modbus devices: Ampowr Ampi Home 3 Phase — Energy meter
• energy_fed_total (number, read-only)
Amount of energy fed to the power grid over lifetime.
Unit: 1 Wh (with accuracy of 10 Wh)
• phase_1.active_power (number, read-only)
...
• color (string)
• icon (string)
366 Modbus devices: Ampowr Ampi Home 3 Phase — Energy meter
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
367 Modbus devices: Daikin Altherma — Heat pump
...
• heat_demand (boolean)
Informs device that heat is demanded or not.
368 Modbus devices: Daikin Altherma — Heat pump
Note
Cannot be modified when device is associated with Heat Pump Manager.
• cool_demand (boolean)
Informs device that cool is demanded or not.
...
369 Modbus devices: Daikin Altherma — Heat pump
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
370 Modbus devices: Daikin Altherma — Main DHW
...
• name (string)
• room_id (integer, read-only, optional)
371 Modbus devices: Daikin Altherma — Main DHW
• status (string, read-only)
• tags (string[])
• type (string, read-only) = "common_dhw_main"
• variant (string, read-only) = "daikin_altherma"
...
372 Modbus devices: Daikin Altherma — Temperature sensor
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
373 Modbus devices: Eastron SDM630 — Energy meter
...
Unit: 1 Wh
374 Modbus devices: Eastron SDM630 — Energy meter
...
Unit: 1 mVA
375 Modbus devices: Eastron SDM630 — Energy meter
...
Unit: 1 varh
376 Modbus devices: Eastron SDM630 — Energy meter
...
Unit: 1 Wh
377 Modbus devices: Eastron SDM630 — Energy meter
...
Unit: 1 varh
378 Modbus devices: Eastron SDM630 — Energy meter
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
379 Modbus devices: EcoAir — Heat pump
EcoAir — Heat pump
...
Condensation temperature.
Unit: 0.1 °C.
380 Modbus devices: EcoAir — Heat pump
• evaporation_temperature (number, read-only)
Evaporation temperature.
Unit: 0.1 °C.
• running_hours (number, read-only)
...
• status (string, read-only)
• tags (string[])
381 Modbus devices: EcoAir — Heat pump
• type (string, read-only) = "heat_pump"
...
382 Modbus devices: EcoAir — Main DHW
EcoAir — Main DHW
...
• room_id (integer, read-only, optional)
• status (string, read-only)
383 Modbus devices: EcoAir — Main DHW
• tags (string[])
• type (string, read-only) = "common_dhw_main"
• variant (string, read-only) = "eco_air"
• visible (boolean, read-only)
...
384 Modbus devices: EcoGeo — Heat pump
EcoGeo — Heat pump
...
Heating return temperature.
Unit: 0.1 °C
385 Modbus devices: EcoGeo — Heat pump
• heating_system_pressure (number, read-only)
Heating System pressure.
Unit: 0.1 bar
• hot_gas_temperature (number, read-only)
...
• color (string)
• icon (string)
386 Modbus devices: EcoGeo — Heat pump
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
...
• reset_alarms
Sends request to heat pump device to reset alarms.
387 Modbus devices: EcoGeo — Main DHW
EcoGeo — Main DHW
...
• room_id (integer, read-only, optional)
• status (string, read-only)
388 Modbus devices: EcoGeo — Main DHW
• tags (string[])
• type (string, read-only) = "common_dhw_main"
• variant (string, read-only) = "eco_geo"
• visible (boolean, read-only)
...
389 Modbus devices: EcoGeo HighPower — Heat pump
...
Heating return temperature.
Unit: 0.1 °C
390 Modbus devices: EcoGeo HighPower — Heat pump
• heating_system_pressure (number, read-only)
Heating System pressure.
...
Note
Cannot be modified when device is associated with Heat Pump Manager.
391 Modbus devices: EcoGeo HighPower — Heat pump
Device properties (full spec)
• class (string, read-only) = "modbus"
• color (string)
• icon (string)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
392 Modbus devices: EcoGeo HighPower — Main DHW
...
• room_id (integer, read-only, optional)
• status (string, read-only)
393 Modbus devices: EcoGeo HighPower — Main DHW
• tags (string[])
• type (string, read-only) = "common_dhw_main"
• variant (string, read-only) = "eco_geo_high_power"
• visible (boolean, read-only)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
394 Modbus devices: Galmet Prima — Heat pump
...
Unit: 0.1 °C
395 Modbus devices: Galmet Prima — Heat pump
...
396 Modbus devices: Galmet Prima — Heat pump
Note
Cannot be modified when device is associated with Heat Pump Manager.
...
Unit: 1 Wh
397 Modbus devices: Galmet Prima — Heat pump
...
• class (string, read-only) = "modbus"
• color (string)
398 Modbus devices: Galmet Prima — Heat pump
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
...
399 Modbus devices: Galmet Prima — Heat pump
...
400 Modbus devices: Galmet Prima — Heat pump
...
401 Modbus devices: Galmet Prima — Heat pump
...
402 Modbus devices: Galmet Prima — Main DHW
...
• icon (string)
• id (integer, read-only)
403 Modbus devices: Galmet Prima — Main DHW
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
• room_id (integer, read-only, optional)
...
404 Modbus devices: Galmet Prima — Temperature sensor
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
405 Modbus devices: GoodWe MT / SMT — Inverter
...
• pv_2.active_power (number, read-only)
Current power produced by second group of photovoltaic panels.
406 Modbus devices: GoodWe MT / SMT — Inverter
Unit: 1 mW
• pv_2.voltage (number, read-only)
Current voltage on second group of photovoltaic panels.
Unit: 1 mV
...
Current voltage on second phase of power grid.
Unit: 1 mV
407 Modbus devices: GoodWe MT / SMT — Inverter
• phase_2.current (number, read-only)
Current current on second phase of power grid.
Unit: 1 mA
• phase_3.voltage (number, read-only)
...
• turn_on
Turns on inverter. (Set power limit to 100 %)
408 Modbus devices: GoodWe MT / SMT — Inverter
• turn_off
Turns off inverter. (Set power limit to 0 %)
• limit_active_power
Sets current active power limit.
...
Unit: 1 %
409 Modbus devices: GoodWe SDT / MS / DNS / XS — Inverter
...
Current voltage on second group of photovoltaic panels.
Unit: 1 mV
410 Modbus devices: GoodWe SDT / MS / DNS / XS — Inverter
• pv_2.current (number, read-only)
Current current on second group of photovoltaic panels.
Unit: 1 mA
• phase_1.voltage (number, read-only)
...
• status (string, read-only)
• tags (string[])
411 Modbus devices: GoodWe SDT / MS / DNS / XS — Inverter
• type (string, read-only) = "inverter"
...
Active power limit (number).
Unit: 1 %
412 Modbus devices: Heatcomp — Heat pump
Heatcomp — Heat pump
...
Evaporation temperature.
Unit: 0.1 °C
413 Modbus devices: Heatcomp — Heat pump
• running_hours (number, read-only)
Hours heat pump is working.
...
Unit: 1 Hz
414 Modbus devices: Heatcomp — Heat pump
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
415 Modbus devices: Heatcomp — Main DHW
Heatcomp — Main DHW
...
• room_id (integer, read-only, optional)
• status (string, read-only)
416 Modbus devices: Heatcomp — Main DHW
• tags (string[])
• type (string, read-only) = "heat_pump"
• variant (string, read-only) = "heatcomp"
• visible (boolean, read-only)
...
417 Modbus devices: Heatcomp HC-EV01 — Car charger
...
Unit: 0.1 °C
418 Modbus devices: Heatcomp HC-EV01 — Car charger
...
• voice_assistant_device_type (string, read-only)
• purpose (string)
419 Modbus devices: Heatcomp HC-EV01 — Car charger
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
Commands
• start_charge
...
Range: 0 h – 15 h.
420 Modbus devices: Heatcomp inverter — Inverter
...
Unit: 1 Wh (with accuracy of 10 Wh)
421 Modbus devices: Heatcomp inverter — Inverter
• energy_consumed_today (number, read-only)
...
• phase_2.active_power (number, read-only)
Current power on second phase of power grid.
422 Modbus devices: Heatcomp inverter — Inverter
Unit: 1 mW
• phase_2.voltage (number, read-only)
Current voltage on second phase of power grid.
Unit: 1 mV
...
• voice_assistant_device_type (string, read-only)
• purpose (string)
423 Modbus devices: Heatcomp inverter — Inverter
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
Commands
• turn_on
...
• turn_off
Turns device off.
424 Modbus devices: Heatcomp inverter — Battery
...
Amount of energy consumed from the battery over a lifetime.
Unit: 1 Wh (with accuracy of 100 Wh).
425 Modbus devices: Heatcomp inverter — Battery
• energy_discharged_today (number, read-only)
Amount of energy consumed from the battery today.
Unit: 1 Wh (with accuracy of 100 Wh).
...
changed when charge_work_mode is set to schedule .
426 Modbus devices: Heatcomp inverter — Battery
...
• status (string, read-only)
• tags (string[])
427 Modbus devices: Heatcomp inverter — Battery
• type (string, read-only) = "battery"
• variant (string, read-only) = "heatcomp_inverter"
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
...
packed arguments (table)
◦ active power in mW (number)
428 Modbus devices: Heatcomp inverter — Battery
• minumum: 0
• maximum: 65000000
...
429 Modbus devices: HeatEco — Heat pump
HeatEco — Heat pump
...
Water inlet temperature.
Unit: 0.1 °C.
430 Modbus devices: HeatEco — Heat pump
...
431 Modbus devices: HeatEco — Heat pump
...
mode.
432 Modbus devices: HeatEco — Heat pump
Note
Cannot be modified when device is associated with Heat Pump Manager.
433 Modbus devices: HeatEco — Main DHW
HeatEco — Main DHW
...
• icon (string)
• id (integer, read-only)
434 Modbus devices: HeatEco — Main DHW
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
• room_id (integer, read-only, optional)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
435 Modbus devices: Huawei SUN2000 — Battery
...
436 Modbus devices: Huawei SUN2000 — Battery
Unit: 0.1 %
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
437 Modbus devices: Huawei SUN2000 — Battery
Commands
• charge
Calls battery to charge during given period of time.
Note
...
Note
Available when device has label battery_charge_support .
438 Modbus devices: Huawei SUN2000 — Battery
Examples
...
439 Modbus devices: Huawei SUN2000 — Inverter
...
Unit: 1 mW
440 Modbus devices: Huawei SUN2000 — Inverter
...
First phase current.
Unit: 1 mA
441 Modbus devices: Huawei SUN2000 — Inverter
...
• icon (string)
• id (integer, read-only)
442 Modbus devices: Huawei SUN2000 — Inverter
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
• room_id (integer, read-only, optional)
...
provided.
Argument:
443 Modbus devices: Huawei SUN2000 — Inverter
...
444 Modbus devices: Huawei SUN2000 — Energy meter
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
445 Modbus devices: Itho — Heat pump
Itho — Heat pump
...
Hot gas temperature.
Unit: 0.1 °C.
446 Modbus devices: Itho — Heat pump
• condensation_temperature (number, read-only)
Condensation temperature.
Unit: 0.1 °C.
• evaporation_temperature (number, read-only)
...
• electric_heater_active (boolean)
Indicates electric heater active state.
447 Modbus devices: Itho — Heat pump
• running_hours (number, read-only)
Hours heat pump is working.
• operating_hours_heating (number, read-only)
Operating hours for central heating.
...
• name (string)
• room_id (integer, read-only, optional)
448 Modbus devices: Itho — Heat pump
• status (string, read-only)
• tags (string[])
• type (string, read-only) = "heat_pump"
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
449 Modbus devices: Itho — Main DHW
Itho — Main DHW
...
• class (string, read-only) = "modbus"
• color (string)
450 Modbus devices: Itho — Main DHW
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
451 Modbus devices: Itho — Temperature sensor
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
452 Modbus devices: Kaisai KHC — Heat pump
...
Water inlet temperature.
Unit: 0.1 °C
453 Modbus devices: Kaisai KHC — Heat pump
• water_outlet_temperature (number, read-only)
Water outlet temperature.
Unit: 0.1 °C
• running_hours (number, read-only)
...
• zone_2.cool_demand (boolean)
Informs device that cool is demanded or not for zone 2..
454 Modbus devices: Kaisai KHC — Heat pump
Note
Cannot be modified when device is associated with Heat Pump Manager.
• zone_2.fixed_target_temperature (number)
Set temperature for heating or cooling in fixed temperature mode for zone 2..
...
Total energy generated by heat pump.
Unit: 1 Wh
455 Modbus devices: Kaisai KHC — Heat pump
• outdoor_unit_capacity (number, read-only)
Outdoor unit power capacity.
Unit: 1 W
• water_flow (number, read-only)
...
• class (string, read-only) = "modbus"
• color (string)
456 Modbus devices: Kaisai KHC — Heat pump
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
...
◦ value - target temperature (number):
• minimum: fixed_target_temperature_minimum
457 Modbus devices: Kaisai KHC — Heat pump
• maximum: fixed_target_temperature_maximum
• unit: °C
• set_heat_demand
Set heat demand for requested zone.
...
• maximum: 9
• set_heat_curve_enabled
458 Modbus devices: Kaisai KHC — Heat pump
Enable or disable heat curve mode for requested zone.
Note
Cannot be executed when heat pump is associated with Heat pump manager.
Arguments:
...
modbus[1]:call("set_cool_demand", { zone=1, value=false })
modbus[1]:call("set_cool_demand", { zone=2, value=false })
459 Modbus devices: Kaisai KHC — Heat pump
Enable and set heat curve id for both zones
modbus[1]:call("set_heat_curve_enabled", { zone=1, value=true })
modbus[1]:call("set_heat_curve", { zone=1, value=2 })
modbus[1]:call("set_heat_curve_enabled", { zone=2, value=true })
...
Activate electric heater
modbus[1]:call("set_electric_heater_active", true)
460 Modbus devices: Kaisai KHC — Main DHW
Kaisai KHC — Main DHW
...
• icon (string)
• id (integer, read-only)
461 Modbus devices: Kaisai KHC — Main DHW
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
• room_id (integer, read-only, optional)
...
462 Modbus devices: Kaisai KHC — Temperature sensor
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
463 Modbus devices: Mitsubishi Ecodan — Heat pump
...
464 Modbus devices: Mitsubishi Ecodan — Heat pump
Note
Cannot be modified when device is associated with Heat Pump Manager.
...
465 Modbus devices: Mitsubishi Ecodan — Heat pump
...
• labels (string[], read-only)
• messages (sequence, read-only)
466 Modbus devices: Mitsubishi Ecodan — Heat pump
• name (string)
• room_id (integer, read-only, optional)
• status (string, read-only)
• tags (string[])
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
467 Modbus devices: Mitsubishi Ecodan — Main DHW
...
• class (string, read-only) = "modbus"
• color (string)
468 Modbus devices: Mitsubishi Ecodan — Main DHW
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
469 Modbus devices: Remeha Elga ACE — Heat pump
...
Heating return temperature.
Unit: 0.1 °C.
470 Modbus devices: Remeha Elga ACE — Heat pump
• heating_system_pressure (number, read-only)
Heating system pressure.
Unit: Bar with one decimal number, multiplied by 10.
• energy_used_for_heating (number, read-only)
...
smart_grid_support ).
471 Modbus devices: Remeha Elga ACE — Heat pump
Device properties (full spec)
• class (string, read-only) = "modbus"
• color (string)
• icon (string)
...
472 Modbus devices: Remeha Elga ACE — Heat pump
Examples
...
473 Modbus devices: Remeha Elga ACE — Temperature sensor
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
474 Modbus devices: SolarEdge with MTTP extension model — Inverter
...
Instantaneous current on first group of photovoltaic panels.
Unit: 1 mA
475 Modbus devices: SolarEdge with MTTP extension model — Inverter
• pv_2.active_power (number, read-only)
Current power produced by second group of photovoltaic panels.
Unit: 1 mW
• pv_2.voltage (number, read-only)
...
• color (string)
• icon (string)
476 Modbus devices: SolarEdge with MTTP extension model — Inverter
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
...
Requires advanced_power_control_enabled to be set to true and
reactive_power_config to rrcr .
477 Modbus devices: SolarEdge — Inverter
SolarEdge — Inverter
...
Reactive power config. Available values are: fixed_cosphi , fixed_q , cosphi , q ,
rrcr .
478 Modbus devices: SolarEdge — Inverter
• active_power_limit (number)
Percent of max power at which inverter is going to work.
Unit: 1 %
Note
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
479 Modbus devices: SolarEdge — Inverter
Commands
• turn_on
Turns on inverter (set power limit to maximum).
Note
...
Requires advanced_power_control_enabled to be set to true and
reactive_power_config to rrcr .
480 Modbus devices: Solax X1 — Battery
Solax X1 — Battery
...
• class (string, read-only) = "modbus"
• color (string)
481 Modbus devices: Solax X1 — Battery
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
...
◦ active power in mW (number)
• minumum: 10000
482 Modbus devices: Solax X1 — Battery
...
483 Modbus devices: Solax X1 — Inverter
Solax X1 — Inverter
...
Current power produced by second group of photovoltaic panels.
Unit: 1 mW
484 Modbus devices: Solax X1 — Inverter
• pv_2.voltage (number, read-only)
Current voltage on second group of photovoltaic panels.
Unit: 1 mV
• pv_2.current (number, read-only)
...
• turn_on
Turns on inverter (set power limit to maximum).
485 Modbus devices: Solax X1 — Inverter
• turn_off
Turns off inverter (set power limit to 0).
486 Modbus devices: Solax X1 — Energy meter
...
meter_number is equal to 1.
Unit: 1 mV
487 Modbus devices: Solax X1 — Energy meter
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
488 Modbus devices: Solax X3 — Battery
Solax X3 — Battery
...
• class (string, read-only) = "modbus"
• color (string)
489 Modbus devices: Solax X3 — Battery
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
...
◦ active power in mW (number)
• minumum: 10000
490 Modbus devices: Solax X3 — Battery
• maximum: from parameter maximum_discharge_power
◦ duration time in seconds (number)
• minumum: 0
• maximum: 65535
...
491 Modbus devices: Solax X3 — Inverter
Solax X3 — Inverter
...
Current power produced by second group of photovoltaic panels.
Unit: 1 mW
492 Modbus devices: Solax X3 — Inverter
• pv_2.voltage (number, read-only)
Current voltage on second group of photovoltaic panels.
Unit: 1 mV
• pv_2.current (number, read-only)
...
• turn_on
Turns on inverter (set power limit to maximum).
493 Modbus devices: Solax X3 — Inverter
• turn_off
Turns off inverter (set power limit to 0).
494 Modbus devices: Solax X3 — Energy meter
...
meter_number is equal to 1.
Unit: 1 mA
495 Modbus devices: Solax X3 — Energy meter
...
Device properties (full spec)
• class (string, read-only) = "modbus"
496 Modbus devices: Solax X3 — Energy meter
• color (string)
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
497 Modbus devices: Solis — Inverter
Solis — Inverter
...
Current voltage on first group of photovoltaic panels.
Unit: 1 mV
498 Modbus devices: Solis — Inverter
• pv_1.current (integer, read-only)
Current current on first group of photovoltaic panels.
Unit: 1 mA
• pv_2.active_power (integer, read-only)
...
Current current on first phase of power grid.
Unit: 1 mA
499 Modbus devices: Solis — Inverter
• phase_2.voltage (integer, read-only)
Current voltage on second phase of power grid.
Unit: 1 mV
• phase_2.current (integer, read-only)
...
• voice_assistant_device_type (string, read-only)
• purpose (string)
500 Modbus devices: Solis — Inverter
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
Commands
• turn_on
...
• turn_off
Turns off inverter (set power limit to 0).
501 Modbus devices: TECH LE-3x230mb — Energy meter
...
Second phase apparent power.
Unit: 1 mVA
502 Modbus devices: TECH LE-3x230mb — Energy meter
• phase_2.reactive_power (number, read-only)
Second phase reactive power.
Unit: 1 mvar
• phase_3.active_power (number, read-only)
...
Unit: 1 Wh
503 Modbus devices: TECH LE-3x230mb — Energy meter
• energy_fed_today (number, read-only)
...
Unit: 1 varh
• monthly_energy_consumption (array, read-only)
Monthly total energy consumption data. Array of objects with parameters:
◦ year (integer) - year of total energy consumption save
◦ month (integer) - month of total energy consumption save
◦ day (integer) - day of total energy consumption save
◦ energy (integer) - total energy consumption for the time of saving the data. Unit:
Wh. Note: this is a total energy consumption value, not month energy consumption.
• energy_consumed_current_month (object, read-only, optional)
Current month energy consumption data. Can be nil when there is no monthly data
saved yet and no total energy consumption data read. Object with parameters:
◦ year (integer) - current year
◦ month (integer) - current month
◦ day (integer) - current day
◦ value (integer) - energy consumption for the current month. Unit: Wh.
• energy_consumed_last_month (object, read-only, optional)
Last month energy consumption data. Can be nil when there is no monthly data saved
for last month. Object with parameters:
◦ year (integer) - year of saving the data for last month
◦ month (integer) - month of saving the data for last month
◦ day (integer) - day of saving the data for last month
◦ value (integer) - energy consumption for the last month. Unit: Wh.
504 Modbus devices: TECH LE-3x230mb — Energy meter
• energy_consumed_month_before_last_month (object, read-only, optional)
Month before the last month energy consumption data. Can be nil when there is no
monthly data saved for month before last month. Object with parameters:
◦ year (integer) - year of saving the data for month before the last month
◦ month (integer) - month of saving the data for month before the last month
◦ day (integer) - day of saving the data for month before the last month
◦ value (integer) - energy consumption for the month before the last month. Unit: Wh.
• monthly_energy_fed (array, read-only)
Monthly total energy fed data. Array of objects with parameters:
◦ year (integer) - year of total energy fed save
◦ month (integer) - month of total energy fed save
◦ day (integer) - day of total energy fed save
◦ value (integer) - total energy fed for the time of saving the data. Unit: Wh. Note:
this is a total energy fed value, not month energy fed.
• energy_fed_current_month (object, read-only, optional)
Current month energy fed data. Can be nil when there is no monthly data saved yet
and no total energy fed data read. Object with parameters:
◦ year (integer) - current year
◦ month (integer) - current month
◦ day (integer) - current day
◦ value (integer) - energy fed for the current month. Unit: Wh.
• energy_fed_last_month (object, read-only, optional)
Last month energy fed data. Can be nil when there is no monthly data saved for last
month. Object with parameters:
◦ year (integer) - year of saving the data for last month
◦ month (integer) - month of saving the data for last month
◦ day (integer) - day of saving the data for last month
◦ value (integer) - energy fed for the last month. Unit: Wh.
• energy_fed_month_before_last_month (object, read-only, optional)
Month before the last month energy fed data. Can be nil when there is no monthly
data saved for month before last month. Object with parameters:
◦ year (integer) - year of saving the data for month before the last month
◦ month (integer) - month of saving the data for month before the last month
◦ day (integer) - day of saving the data for month before the last month
◦ value (integer) - energy fed for the month before the last month. Unit: Wh.
505 Modbus devices: TECH LE-3x230mb — Energy meter
Device properties (full spec)
• class (string, read-only) = "modbus"
• color (string)
• icon (string)
...
• delayed_actions (array, read-only)
Examples
Read the last month energy consumption on the first day of the month
and send notification about the consumption
if dateTime:changed() then
-- check at 10:00AM
if dateTime:getDay() == 1 and dateTime:getHours() == 10 and
dateTime:getMinutes() == 0 then
local meter = modbus[1]
local lastMonth = meter:getValue("energy_consumed_last_month")
if lastMonth ~= nil then
local message = string.format("%u for %04d-%02d", lastMonth.value,
lastMonth.year, lastMonth.month)
notify:info("Energy consumption", message)
end
end
end
506 Modbus devices: KM1 energy meter converter
...
• phase_1.voltage (integer, read-only)
First phase voltage.
507 Modbus devices: KM1 energy meter converter
Unit: 1 mV
• phase_1.current (integer, read-only)
First phase current.
Unit: 1 mA
...
508 Modbus devices: KM1 energy meter converter
...
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
509 Modbus devices: KM1 energy meter converter
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
Commands
...
execution context.
Properties can only be accessed via setValue and getValue methods.
511 Virtual devices: Thermostat
Thermostat
...
512 Virtual devices: Thermostat
• target_temperature (integer)
...
Unit: °C with one decimal number, multiplied by 10.
513 Virtual devices: Thermostat
...
514 Virtual devices: Thermostat
...
515 Virtual devices: Thermostat
Configures whether radiator actuators should be controlled (opening and closing) while
...
516 Virtual devices: Thermostat
...
517 Virtual devices: Thermostat
Device properties (full spec)
...
518 Virtual devices: Thermostat
• enable_constant_mode
...
519 Virtual devices: Thermostat
...
520 Virtual devices: Thermostat output group (virtual contact)
...
521 Virtual devices: Thermostat output group (virtual contact)
• associations.thermostats (array of devices)
Reference to associated thermostats. Returns empty array when no devices associated.
• associations.relays (array of devices)
Reference to associated relays. Returns empty array when no devices associated.
...
• variant (string, read-only) = "generic"
• visible (boolean, read-only)
522 Virtual devices: Thermostat output group (virtual contact)
• voice_assistant_device_type (string, read-only)
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
...
523 Virtual devices: Thermostat output group (virtual contact)
...
524 Virtual devices: Relay integrator
Relay integrator
...
• variant (string, read-only) = "generic"
• visible (boolean, read-only)
525 Virtual devices: Relay integrator
• voice_assistant_device_type (string, read-only)
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
...
526 Virtual devices: Blind controller integrator
...
blind controllers.
The integrator‘s properties and commands availability is dependent on the type of
associated blind controllers.
• If associated blind controllers support opening control (eg. rollershade, tilt blind), the
integrator will support opening control (will gain percent_opening_control label).
• If associated blind controllers support tilt control (titl blind, pergola), the integrator will
also support tilt control (will gain percent_tilt_control label).
Properties
Direct access to properties is not allowed. You can read or change values using getValue
and setValue methods. An attempt at retrieving a nonexistent object property, or setting
wrong value type will cause a script error.
Required label: "percent_opening_control"
...
527 Virtual devices: Blind controller integrator
...
argument. Available only when Integrator has percent_opening_control label.
Argument:
target opening in percent (number)
• up
Sets the target_opening of all associated blind controllers to 100%. Available only
when Integrator has percent_opening_control label.
528 Virtual devices: Blind controller integrator
• down
Sets the target_opening of all associated blind controllers to 0%. Available only when
Integrator has percent_opening_control label.
• stop
...
Available only when Integrator has percent_tilt_control label.
Argument:
...
529 Virtual devices: Custom device
Custom device
...
530 Virtual devices: Custom device
...
Returns:
531 Virtual devices: Custom device
◦ userdata — reference to device object, used for chained calls
Arguments:
...
◦ userdata — reference to device object, used for chained calls
Arguments:
532 Virtual devices: Custom device
...
Attempting to reference a nonexistent object, retrieve a nonexistent object property, or set
the wrong value type will result in a script error.
533 Virtual devices: Custom device
• id (number, read-only)
Unique object identifier.
• name (string)
...
won’t be executed.
534 Virtual devices: Custom device
...
Unit: 1 s
Range: 1 s – 120 s
535 Virtual devices: Custom device
Commands
• enable
Enables device.
• disable
...
536 Virtual devices: Heat pump manager
Heat pump manager
...
537 Virtual devices: Heat pump manager
...
• target_temperature_mode (string)
538 Virtual devices: Heat pump manager
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
539 Virtual devices: Heat pump manager
Commands
• enable
...
Available only when device has label has_schedule .
540 Virtual devices: Heat pump manager
Examples
...
548 Custom devices: Lua reference
Lua reference
...
549 Custom devices: Lua reference
...
Arguments:
567 Custom devices: Elements
◦ string
◦ element_reference
568 Custom devices: Elements
Color picker
...
Arguments:
◦ object-like table
576 Custom devices: Elements
◦ element_reference
577 Custom devices: Elements
Date picker
...
-- add heat pump, DHW, energy meter, temperature sensor
-- and relay modules
self:addModules({
{
type = "heat_pump",
...
--- common callback which will be invoked when any of the dimmers
--- changes parameter
...
--- common callback which will be invoked when any of the dimmers receives the
--- command call
function CustomDevice:onModuleCommand(uuid, cmd, commandParameters,
changedParameters)
if cmd == "set_level" then
614 Custom devices: Modules
...
622 Custom devices: Examples
callback.
623 CustomDeviceModule devices
...
Certain read-only properties are marked as updatable. These properties can be modified
only using updateProperties method.
Common Custom device module device properties
• uuid (string, read-only)
Unique ID if the module in custom device.
Required label: "battery_powered"
• battery (integer, read-only, updatable)
Unit: 1%
Value range: ⟨0; 100⟩
• signal (integer, read-only)
...
624 CustomDeviceModule devices
...
• visible (boolean)
Indicates if device is visible in application.
625 CustomDeviceModule devices: Air quality sensor
Air quality sensor
...
• pm1p0 (number, read-only, updatable)
Measured concentration of PM1.0 value (particulate matter).
Unit: 1 μg/m3
Range: 0 μg/m3 – 1000 μg/m3
• pm2p5 (number, read-only, updatable)
Measured concentration of PM2.5 value (particulate matter).
Unit: 1 μg/m3
Range: 0 μg/m3 – 1000 μg/m3
• pm4p0 (number, read-only, updatable)
Measured concentration of PM4.0 value (particulate matter).
Unit: 1 μg/m3
Range: 0 μg/m3 – 1000 μg/m3
• pm10p0 (number, read-only, updatable)
Measured concentration of PM10.0 value (particulate matter).
Unit: 1 μg/m3
Range: 0 μg/m3 – 1000 μg/m3
• air_quality (string, read-only)
...
111 – 150 unhealthy
> 150 very_unhealthy
626 CustomDeviceModule devices: Air quality sensor
Device properties (full spec)
...
• params_callback_function (string)
• command_callback_function (string)
627 CustomDeviceModule devices: Air quality sensor
Examples
...
628 CustomDeviceModule devices: Analog input
Analog input
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• value (number, read-only, updatable)
Measured analog value.
Range: ⟨−263; 263⟩
• unit (string)
Unit of the value, used for displaying and statistics.
Can contain only alphanumeric characters and - , _ . Pattern: [a-zA-Z0-9\\-_]+ .
Max length: 16
Example: "mV"
• value_valid (boolean, read-only, updatable)
...
629 CustomDeviceModule devices: Analog input
• variant (string, read-only) = "generic"
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
• purpose (string)
...
630 CustomDeviceModule devices: Analog Output
Analog Output
...
• value_minimum (integer, read-only, updatable)
Lower limit of value.
Lower limit: −65 535
Upper limit: value_minimum
Note: Parameter can be changed only using updateProperties from containing
custom device.
• value_maximum (integer, read-only, updatable)
Upper limit of value.
Lower limit: value_minimum
Upper limit: 65 535
Note: Parameter can be changed only using updateProperties from containing
custom device.
• unit (string, read-only, updatable)
Unit of the value, used for displaying and statistics. Can contain only alphanumeric
characters and - , _ . Pattern: [a-zA-Z0-9\\-_]+ .
Max length: 16
Example: mV
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
...
• icon (string)
• id (integer, read-only)
631 CustomDeviceModule devices: Analog Output
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
• room_id (integer, read-only, optional)
...
value_minimum = 100,
value_maximum = 1500,
632 CustomDeviceModule devices: Analog Output
unit = "mA"
}
})
end
...
633 CustomDeviceModule devices: Battery
Battery
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• charge_power (integer, read-only, updatable)
Current charing (positive number) or discharging (negative number) power.
Unit: 1 mW
Range: ⟨−263; 263⟩ Wh
• soc (integer, read-only, updatable)
Current state of charge.
Unit: 1 %
Range: 0 % – 100 %
• energy_charged_today (integer, read-only, updatable)
Amount of energy charged to the battery today.
Unit: 1 Wh (with accuracy of 100 Wh).
Range: ⟨0; 263⟩ Wh
• energy_discharged_today (integer, read-only, updatable)
Amount of energy consumed from the battery today.
Unit: 1 Wh (with accuracy of 100 Wh)
Range: ⟨0; 263⟩ Wh
• battery_charge_support (boolean, read-only, updatable)
Indicates if battery charge commands should be supported. Adds or removes the label
...
634 CustomDeviceModule devices: Battery
Note
...
Available when device has label battery_charge_support . Parameter
battery_charge_support must be set to true .
635 CustomDeviceModule devices: Battery
Examples
Create the module and update properties in custom device code
function CustomDevice:onInit(reason)
-- create only when added or imported
...
--- command callback function to update remote module after command call on
--- battery in Sinum
...
636 CustomDeviceModule devices: Blind controller
Blind controller
...
to 0%, 50% or 100% (only these three).
• current_opening (integer, read-only, updatable)
Current opening value.
Unit: 1 %
Range: 0 % – 100 %
Required label: "percent_tilt_control"
...
Unit: 1 %
Range: 0 % – 100 %
• current_tilt (integer, optional, read-only, updatable)
Current tilt position
Unit: 1 %
Range: 0 % – 100 %
• percent_control_supported (boolean, read-only, updatable)
Indicates if percent control is supported for the controller. Adds the
percent_opening_control label if set to true .
637 CustomDeviceModule devices: Blind controller
Default: true
• tilt_control_supported (boolean, read-only, updatable)
Indicates if tilt control is supported for the controller. Adds the
percent_tilt_control label if set to true .
Default: true
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
• color (string)
...
• software_version (string, read-only)
• params_callback_function (string)
638 CustomDeviceModule devices: Blind controller
• command_callback_function (string)
Commands
• open
Opens a blind to specific value in percent passed in argument.
...
639 CustomDeviceModule devices: Blind controller
-- callback function to update remote module after changes in Sinum
...
640 CustomDeviceModule devices: Blind controller
Set blind to half-open at noon
if dateTime:changed() then
if dateTime:getHours() == 12 and dateTime:getMinutes() == 0 then
...
641 CustomDeviceModule devices: Car Charger
Car Charger
...
Unit: 1 mW
Range: 0 mW – 2 147 483 647 mW
• energy_charged_total (integer, read-only, updatable)
Total energy charged during device lifetime. Used to show energy consumption for car
chargers in Energy consumption widget. Today’s energy is calculated based on changes
of this parameter.
Unit: 1 Wh
...
• room_id (integer, read-only, optional)
• status (string, read-only)
642 CustomDeviceModule devices: Car Charger
• tags (string[])
...
-- create only when added or imported
if reason == "add" or reason == "import" then
643 CustomDeviceModule devices: Car Charger
self:addModule({
type = "car_charger",
...
644 CustomDeviceModule devices: CO2 sensor
CO2 sensor
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• co2 (integer, read-only, updatable)
Measured CO2 value.
Unit: 1 PPM
Range: 0 PPM – 10000 PPM
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
• color (string)
• icon (string)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
645 CustomDeviceModule devices: CO2 sensor
Custom device modules properties (full spec)
• uuid (string, read-only)
• battery (integer, read-only)
• signal (integer, read-only)
...
646 CustomDeviceModule devices: Dimmer
Dimmer
Device that controlls light intensity of output LED.
Properties
Direct access to properties is not allowed. You can read or change values using getValue ,
...
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
647 CustomDeviceModule devices: Dimmer
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
Custom device modules properties (full spec)
...
648 CustomDeviceModule devices: Dimmer
Examples
Create the module and update properties in custom device code
function CustomDevice:onInit(reason)
-- create only when added or imported
...
local transitionTime = commandParameters[2] or 50
-- send the update to remote device
end
end
-- update module state in event loop with values read from device
649 CustomDeviceModule devices: Dimmer
function CustomDevice:onEvent(event)
-- read the data from remote device
...
650 CustomDeviceModule devices: Domestic Hot Water
Domestic Hot Water
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• current_temperature (number, read-only, updatable)
Current domestic hot water temperature.
Unit: 0.1 °C
Range: −400 – 1500 (−40.0 °C – 150.0 °C)
• target_temperature (number)
...
• class (string, read-only) = "custom_device_module"
• color (string)
651 CustomDeviceModule devices: Domestic Hot Water
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
...
Argument:
target temperature in 0.1°C (number)
652 CustomDeviceModule devices: Domestic Hot Water
Examples
Create the module and update properties in custom device code
function CustomDevice:onInit(reason)
-- create only when added or imported
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• total_active_power (integer, read-only, updatable)
Active power measured by energy meter.
Unit: 1 mW
Range: ⟨−263; 263⟩ mW
• energy_sum_total (integer, read-only, updatable)
Total energy consumed of all phases lifetime. Used to show energy consumption for
electrical sockets in Energy consumption widget. Today’s energy is calculated based on
changes of this parameter. Parameter is only used for displaying electrical sockets’
energy, so only if uses_energy_of_building is true .
Unit: 1 Wh
Range: ⟨0; 263⟩ Wh
• uses_energy_of_building (boolean, read-only, updatable)
Indicates if energy meter is used to measure energy of device inside the building. (Not
main energy meter). When set to true it is automatically included in Energy center
widgets calculations (Flow monitor and Energy consumption). Should be set to false if
it is used as main energy meter. In this case to be included in Energy center widgets
...
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
654 CustomDeviceModule devices: Energy meter
• color (string)
• icon (string)
• id (integer, read-only)
• labels (string[], read-only)
...
if reason == "add" or reason == "import" then
self:addModule({
655 CustomDeviceModule devices: Energy meter
type = "energy_meter",
...
656 CustomDeviceModule devices: Flood sensor
Flood sensor
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• flood_detected (boolean, read-only, updatable)
A flag representing the detection of flood / water leak by the sensor.
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
• color (string)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
657 CustomDeviceModule devices: Flood sensor
Custom device modules properties (full spec)
• uuid (string, read-only)
• battery (integer, read-only)
• signal (integer, read-only)
...
658 CustomDeviceModule devices: Heat pump
Heat pump
...
• id (integer, read-only)
• labels (string[], read-only)
659 CustomDeviceModule devices: Heat pump
• messages (sequence, read-only)
• name (string)
• room_id (integer, read-only, optional)
• status (string, read-only)
...
-- callback function to update remote module after changes in Sinum
660 CustomDeviceModule devices: Heat pump
...
661 CustomDeviceModule devices: Humidity sensor
Humidity sensor
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• humidity (integer, read-only, updatable)
Measured relative humidity value.
Unit: 0.1 %
Range: 0 – 1000 (0.0 % – 100.0 %)
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
662 CustomDeviceModule devices: Humidity sensor
Custom device modules properties (full spec)
• uuid (string, read-only)
• battery (integer, read-only)
• signal (integer, read-only)
...
663 CustomDeviceModule devices: IAQ sensor
IAQ sensor
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• iaq (integer, read-only, updatable)
Calculated Index of Air Quality.
Range: 0 – 1000
• air_quality (string, read-only)
Descriptive name for air quality.
...
• labels (string[], read-only)
• messages (sequence, read-only)
664 CustomDeviceModule devices: IAQ sensor
• name (string)
• room_id (integer, read-only, optional)
• status (string, read-only)
• tags (string[])
...
665 CustomDeviceModule devices: IAQ sensor
iaq = iaqMeasured
})
end
666 CustomDeviceModule devices: Inverter
Inverter
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• pv_total_active_power (integer, read-only, updatable)
Current total power produced by all photovoltaic panels. Parameter will be used to
calculate power flow to grid, battery and house in Flow monitor.
Unit: 1 mW
Range: ⟨0; 263⟩ mW
• energy_produced_total (integer, read-only, updatable)
Total amount of energy produced by PV over a lifetime. Parameter will be used to
display energy distibution in Energy consumption widget. Energy used for
auto-consumption, energy storage and grid. Distribution is calculated using this
parameter and ratio based on flow monitor power flow.
Unit: 1 Wh
Range: ⟨0; 263⟩ Wh
• power_to_grid (integer, read-only, updatable)
Current power fed to (positive number) or consumed from (negative number) the power
grid.
Unit: 1 mW
Range: ⟨−263; 263⟩ Wh
...
• name (string)
• room_id (integer, read-only, optional)
667 CustomDeviceModule devices: Inverter
• status (string, read-only)
• tags (string[])
• type (string, read-only) = "inverter"
• variant (string, read-only) = "generic"
...
Calls a command callback. No action is actualy performed on the device parameters.
Argument:
Active power limit (integer)
Unit: 1 mW
Range: ⟨0; 263⟩ mW
668 CustomDeviceModule devices: Inverter
Examples
Create the module and update properties in custom device code
function CustomDevice:onInit(reason)
-- create only when added or imported
...
669 CustomDeviceModule devices: Light sensor
Light sensor
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• illuminance (integer, read-only, updatable)
Measured light illuminance value.
Unit: 1 lx
Range: 0 lx – 65 535 lx
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
• color (string)
• icon (string)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
670 CustomDeviceModule devices: Light sensor
Custom device modules properties (full spec)
• uuid (string, read-only)
• battery (integer, read-only)
• signal (integer, read-only)
...
671 CustomDeviceModule devices: Motion sensor
Motion sensor
...
will cause a script error.
• enabled (boolean)
Enable or disable sensor. e.g. If you want sense only at nighttime, you can set up an
automation to enable/disable sensor.
• motion_detected (boolean, read-only, updatable)
...
• tags (string[])
• type (string, read-only) = "motion_sensor"
672 CustomDeviceModule devices: Motion sensor
• variant (string, read-only) = "generic"
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
• purpose (string)
...
Adds additional emitting time_since_motion event in seconds passed in argument.
Argument:
Event reemission delay (integer)
Unit: 1 s
Minimum: 1 s
Note
It does not call the module command callback as it does not affect the device
directly.
673 CustomDeviceModule devices: Motion sensor
Examples
Create the module and update properties in custom device code
function CustomDevice:onInit(reason)
-- create only when added or imported
...
674 CustomDeviceModule devices: Motion sensor
Delayed action
if dateTime:changed() then
-- add 30 second delay
...
675 CustomDeviceModule devices: Opening sensor
Opening sensor
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• open (boolean, read-only, updatable)
Opening sensor state. Open/Closed.
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
• color (string)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
676 CustomDeviceModule devices: Opening sensor
Custom device modules properties (full spec)
• uuid (string, read-only)
• battery (integer, read-only)
• signal (integer, read-only)
...
677 CustomDeviceModule devices: Pressure sensor
Pressure sensor
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• pressure (integer, read-only, updatable)
Measured pressure value.
Unit: 10 Pa (hPa with one decimal number, multiplied by 10)
Range: 3000 – 11 000 (300 hPa – 1100 hPa)
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
• color (string)
• icon (string)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
678 CustomDeviceModule devices: Pressure sensor
Custom device modules properties (full spec)
• uuid (string, read-only)
• battery (integer, read-only)
• signal (integer, read-only)
...
679 CustomDeviceModule devices: RGB controller
RGB controller
Device that controlls color and light intensity of output LED.
Properties
Direct access to properties is not allowed. You can read or change values using getValue ,
...
HTML/Hex RGB color that device will set on its output led strip.
Example: "#00ff7f"
• white_temperature (integer, read-only)
White temperature that device will set on its output led strip.
Unit: 1 K
• color_mode (string, read-only)
...
• name (string)
• room_id (integer, read-only, optional)
680 CustomDeviceModule devices: RGB controller
• status (string, read-only)
• tags (string[])
• type (string, read-only) = "rgb_controller"
• variant (string, read-only) = "generic"
...
• maximum: 100
◦ transition time in 0.1s (number):
681 CustomDeviceModule devices: RGB controller
• minimum: 1 (100 ms)
• maximum: 6000 (10 minutes)
• parameter is optional (500 ms default)
• set_color
...
682 CustomDeviceModule devices: RGB controller
end
end
-- callback function to update remote module after changes in Sinum
...
--- command callback function to update remote
--- module after command call on controller in Sinum
...
if dateTime:changed() then
if dateTime:getHours() == 19 and dateTime:getMinutes() == 0 then
683 CustomDeviceModule devices: RGB controller
rgb:call("set_color", { "#eedd11", 10 })
elseif dateTime:getHours() == 21 and dateTime:getMinutes() == 0 then
rgb:call("turn_off")
end
...
684 CustomDeviceModule devices: Radiator actuator
Radiator actuator
...
Current valve opening level.
Unit: 1 %
Minimum: opening_minimum
Maximum: opening_maximum
• opening_minimum (integer)
Lower valve opening level limit. Could not be greater than maximum. Setting minimum
value above current opening value, will also change current opening value to minimum.
Unit: 1 %
Minimum: 0 %
Maximum: opening_maximum
Note
Does not call parameter change callback as it does not affect device directly.
• opening_maximum (integer)
Upper valve opening level limit. Could not be less than minimum. Setting maximum
value below current opening value, will also change current opening value to maximum.
Unit: 1 %
Minimum: opening_minimum
...
Note
Does not call parameter change callback as it does not affect device directly.
• emergency_opening (integer)
Emergency opening level when communication with central device is lost.
Unit: 1 %
Range: 0 % – 100 %
685 CustomDeviceModule devices: Radiator actuator
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
• color (string)
• icon (string)
...
• open
Opens radiator actuator to desired value in percent passed in argument.
686 CustomDeviceModule devices: Radiator actuator
Argument:
Actuator opening (integer)
Unit: 1 %
Minimum: opening_minimum
Maximum: opening_maximum
• calibration
...
687 CustomDeviceModule devices: Radiator actuator
Regulate valve based on room temperature
local sensor = wtp[1]
...
688 CustomDeviceModule devices: Relay
Relay
Execution module that changes state depending on the control signal. Relay can be
assigned to virtual thermostat in web application.
Properties
...
• color (string)
• icon (string)
689 CustomDeviceModule devices: Relay
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
...
• toggle
Changes relay output to opposite.
690 CustomDeviceModule devices: Relay
Examples
Create the module and update properties in custom device code
function CustomDevice:onInit(reason)
-- create only when added or imported
...
691 CustomDeviceModule devices: Smoke sensor
Smoke sensor
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• smoke_detected (boolean, read-only, updatable)
A flag representing the detection of smoke by the sensor.
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
• color (string)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
692 CustomDeviceModule devices: Smoke sensor
Custom device modules properties (full spec)
• uuid (string, read-only)
• battery (integer, read-only)
• signal (integer, read-only)
...
693 CustomDeviceModule devices: Smoke sensor
})
end
Reacting to smoke
local fan = wtp[2]
...
694 CustomDeviceModule devices: Temperature regulator
Temperature regulator
...
Desired setpoint temperature, which device will try to achieve.
Unit: 0.1 °C
Minimum: target_temperature_minimum
Maximum: target_temperature_maximum
• target_temperature_mode.current (string, read-only)
Regulator target temperature mode. Specifies if regulator works in constant mode
with one target temperature, time_limited mode with one temporary target
temperature or according to schedule in schedule mode with many target
...
Minimum: 50 (5.0 °C)
Maximum: target_temperature_maximum
695 CustomDeviceModule devices: Temperature regulator
• target_temperature_maximum (integer)
Upper limit of the target temperature. Could not be less than minimum. Setting
maximum below target, will also change target value to minimum.
Unit: 0.1 °C
...
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
696 CustomDeviceModule devices: Temperature regulator
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
Custom device modules properties (full spec)
...
Calls Temperature Regulator to change mode and target temperature mode to
time_limited for desired time.
697 CustomDeviceModule devices: Temperature regulator
When regulator is already in time_limited mode, it will change remaining_time
or/and target_temperature depending on payload.
Note
Cannot be executed when regulator is not associated with a thermostat.
...
params_callback_function = "onTemperatureRegulatorModuleChanged",
...
698 CustomDeviceModule devices: Temperature regulator
local targetTemperature = commandParameters
-- send the update to remote device
...
-- send target temperature change
-- from parameter targetTemperatureChanged
end
if modeChanged then
...
699 CustomDeviceModule devices: Temperature sensor
Temperature sensor
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• temperature (integer, read-only, updatable)
Measured temperature value.
Unit: 0.1 °C
Range: −1000 – 3000 (−100.0 °C – 300.0 °C)
Device properties (full spec)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
700 CustomDeviceModule devices: Temperature sensor
Custom device modules properties (full spec)
• uuid (string, read-only)
• battery (integer, read-only)
• signal (integer, read-only)
...
701 CustomDeviceModule devices: Two-state input sensor
Two-state input sensor
...
call). An attempt at retrieving a nonexistent object property, or setting wrong value type
will cause a script error.
• state (boolean, read-only, updatable)
State of the input.
Device properties (full spec)
• class (string, read-only) = "custom_device_module"
• color (string)
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
702 CustomDeviceModule devices: Two-state input sensor
Custom device modules properties (full spec)
• uuid (string, read-only)
• battery (integer, read-only)
• signal (integer, read-only)
...
• endpoint (integer, read-only)
• software_version (string, read-only)
706 SBUS devices: Analog output
Analog output
Analog output representation. Set desired value to output in device.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
• labels (string[], read-only)
• messages (sequence, read-only)
707 SBUS devices: Analog output
• name (string)
• room_id (integer, read-only, optional)
• status (string, read-only)
• tags (string[])
...
708 SBUS devices: Blind controller
Blind controller
Controller opens and closes a roller shade, tilt blind or pergola.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
Note
Can be modified with values in allowed_lift_control_modes property.
709 SBUS devices: Blind controller
• allowed_lift_control_modes (table, read-only)
List of available lift control modes supported by the controller.
Possible values: current_detection , fixed_duration .
Required label: "percent_tilt_control"
...
Can be modified when: lift_control_mode is equal to fixed_duration .
Required label: "has_tilt_duration"
710 SBUS devices: Blind controller
• tilt_duration (number, optional)
Time required by motor to do full tilt cycle.
Unit: ms.
Required label: "has_tilt_cycle_distance"
...
HTML/Hex RGB representation of color when controller is active e.g. motor is working.
Example: #FFFF00
711 SBUS devices: Blind controller
Device properties (full spec)
• class (string, read-only) = "sbus"
• color (string)
• icon (string)
...
Command is NOT available when window_covering_type is pergola .
Argument:
712 SBUS devices: Blind controller
opening percentage (number)
• up
Fully opens a blind.
Note
...
button:call("emit", { button_index = 2, type = "hold_start", set = 3 })
-- Emit hold end after 10 secs on button 2 in set 3 (channel 3)
-- NOTE: if the previous action was emitted by button in set 3, all subsequent
-- actions will be in same set until the set is changed by remote
-- or by using `set` property of the command
button:call("emit", { button_index = 2, type = "hold_end", duration = 10 })
716 SBUS devices: CO2 sensor
CO2 sensor
Battery powered CO2 sensor. Measures CO2 concentration in the air and sends
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
717 SBUS devices: CO2 sensor
SBUS device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
718 SBUS devices: Dimmer
Dimmer
Device that controlls light intensity of output LED.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
728 SBUS devices: Humidity sensor
Humidity sensor
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
729 SBUS devices: Humidity sensor
SBUS device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
730 SBUS devices: IAQ sensor
IAQ sensor
Battery powered Index of Air Quality sensor. Calculates Air Quality Index based on various
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
733 SBUS devices: Light sensor
SBUS device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
734 SBUS devices: Motion sensor
Motion sensor
...
Time since last motion detected. Value of -1 means there wasn’t any motion since last
system startup.
735 SBUS devices: Motion sensor
Note
The value will be 0 for each detected move, even if the previous one has not yet
finished.
Unit: seconds.
...
Commands
• enable
736 SBUS devices: Motion sensor
Enables motion detector.
• disable
Disables motion detection.
• add_time_since_motion_event
...
737 SBUS devices: Motion sensor
Enable a light for 5 minutes on motion detection
...
738 SBUS devices: Pressure sensor
Pressure sensor
Pressure sensor measures pressure and sends measurement to central unit.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
739 SBUS devices: Pressure sensor
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
SBUS device properties (full spec)
...
• endpoint (integer, read-only)
• software_version (string, read-only)
740 SBUS devices: Pulse width modulation
...
"rgbww" .
743 SBUS devices: RGB controller
• active_animation (number, read-only)
Active animation ID if animation was activated. Null value when no animation active.
Device properties (full spec)
• class (string, read-only) = "sbus"
...
Turns on output.
Argument
744 SBUS devices: RGB controller
Optional: transition time in 0.1s (default 5 - 500ms) (number)
• turn_off
Turns off output.
Argument
...
745 SBUS devices: RGB controller
• minimum: 1 (100 ms)
• maximum: 6000 (10 minutes)
• unit: 100ms
• parameter is optional (500 ms default)
...
746 SBUS devices: RGB controller
Activate an animation by ID
local rgb = sbus[79]
local animation_id = 2
rgb:call("activate_animation", { id = animation_id })
...
751 SBUS devices: Temperature regulator
Temperature regulator
Temperature regulator notifies when desired temperature is reached in room. Can be
assigned to virtual thermostat in web application.
Normally works in constant temperature mode only, but additional modes
...
maximum below target, will also change target value to minimum.
Unit: 0.1 °C
752 SBUS devices: Temperature regulator
• target_temperature_reached (boolean)
Controls device’s algorithm state indicator (available on some regulators). e.g. LED
Diode. May be controlled by external algorithms or devices such as Thermostat (when
thermostat is active, indicator will blink)
...
• color (string)
• icon (string)
753 SBUS devices: Temperature regulator
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
...
target temperature in 0.1°C (number)
• enable_constant_mode
754 SBUS devices: Temperature regulator
Calls Temperature Regulator to change target temperature mode to constant . When
regulator is already in constant mode, it will change mode target_temperature only.
Note
Cannot be executed when regulator is not associated with a thermostat.
...
755 SBUS devices: Temperature sensor
Temperature sensor
...
• voice_assistant_device_type (string, read-only)
• purpose (string)
756 SBUS devices: Temperature sensor
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
SBUS device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
757 SBUS devices: Two state input sensor
Two state input sensor
Boolean input sensor checks input state and send it to central unit.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
• purpose (string)
• supervised_properties (array, read-only)
758 SBUS devices: Two state input sensor
• delayed_actions (array, read-only)
SBUS device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
759 SBUS devices: Valve
Valve
...
• endpoint (integer, read-only)
• software_version (string, read-only)
765 SBUS devices: Valve Analog PID
Valve Analog PID
...
Unit: 0.1 °C.
766 SBUS devices: Valve Analog PID
• proportional_gain (integer)
...
• voice_assistant_device_type (string, read-only)
• purpose (string)
767 SBUS devices: Valve Analog PID
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
SBUS device properties (full spec)
• address (integer, read-only)
...
768 SBUS devices: Water meter
Water meter
Water meter is a device which can track and count total water volumes passed through
meter and current water flow.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
and setValue methods. An attempt at retrieving a nonexistent object property, or setting
wrong value type will cause a script error.
• current_water_flow (number, read-only)
Current water flow.
Unit: 0.01 m3/h
Note
Parameter is optional.
• total_water_flow (number, read-only)
Total water flow during the lifetime of water meter.
Unit: 0.01 m3
• monthly_water_flow (array, read-only)
Monthly water flow data. Array of objects with parameters:
◦ year (integer) - year of total water flow volume save
◦ month (integer) - month of total water flow volume save
◦ day (integer) - day of total water flow volume save
◦ value (integer) - total water flow volume for the time of saving the data. Unit: m3
with two decimal numbers, multiplied by 100. Note: this is a total water flow volume
value, not month water flow volume.
• water_flow_current_month (object, read-only, optional)
Current month water flow volume data. Can be nil when there is no monthly data
saved yet and no total water flow volume data read. Object with parameters:
◦ year (integer) - current year
◦ month (integer) - current month
◦ day (integer) - current day
◦ value (integer) - water flow volume for the current month. Unit: m3 with two
decimal numbers, multiplied by 100..
• water_flow_last_month (object, read-only, optional)
Last month water flow volume data. Can be nil when there is no monthly data saved
for last month. Object with parameters:
769 SBUS devices: Water meter
◦ year (integer) - year of saving the data for last month
◦ month (integer) - month of saving the data for last month
◦ day (integer) - day of saving the data for last month
◦ value (integer) - water flow volume for the last month. Unit: m3 with two decimal
numbers, multiplied by 100..
• water_flow_month_before_last_month (object, read-only, optional)
Month before the last month water flow volume data. Can be nil when there is no
monthly data saved for month before last month. Object with parameters:
◦ year (integer) - year of saving the data for month before the last month
◦ month (integer) - month of saving the data for month before the last month
◦ day (integer) - day of saving the data for month before the last month
◦ value (integer) - water flow volume for the month before the last month. Unit: m3
with two decimal numbers, multiplied by 100..
Device properties (full spec)
• class (string, read-only) = "sbus"
• color (string)
• icon (string)
...
• status (string, read-only)
• tags (string[])
• type (string, read-only) = "water_meter"
• variant (string, read-only) = "generic"
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
770 SBUS devices: Water meter
SBUS device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
Send notification when current flow rises above 150m3/h.
local threshold = 15000
if sbus[10]:changedValue("current_water_flow") then
if sbus[10]:getValue("current_water_flow") > treshold then
notify:error("Current flow too high!", "Check your device!")
end
end
Read the last month water flow volume on the first day of the month and
send notification about the consumption
if dateTime:changed() then
-- check at 10:00AM
if dateTime:getDay() == 1 and dateTime:getHours() == 10 and
dateTime:getMinutes() == 0 then
local meter = sbus[1]
local lastMonth = meter:getValue("water_flow_last_month")
if lastMonth ~= nil then
local message = string.format("%u for %04d-%02d", lastMonth.value,
lastMonth.year, lastMonth.month)
notify:info("Water consumption", message)
end
end
end
771 SBUS devices: Heat meter
Heat meter
Heat meter is a device which can track and count total heat energy consumption.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
and setValue methods. An attempt at retrieving a nonexistent object property, or setting
wrong value type will cause a script error.
• current_water_flow (number, read-only)
Current water flow. Unit: m3/h with two decimal number, multiplied by 100.
Note
Parameter is optional.
• total_water_flow (number, read-only)
Total water flow during the lifetime of water meter. Unit: m3 with two decimal number,
multiplied by 100.
• monthly_water_flow (array, read-only)
Monthly water flow data. Array of objects with parameters:
◦ year (integer) - year of total water flow volume save
...
◦ value (integer) - water flow volume for the month before the last month. Unit: m3
with two decimal numbers, multiplied by 100..
• flow_temperature (number, read-only, optional)
Temperature measured for water flow. Unit: °C with one decimal number, multiplied by
10.
• return_temperature (number, read-only, optional)
Temperature measured on installation return. Unit: °C with one decimal number,
multiplied by 10.
• active_power (number, read-only, optional)
Instantaneous value of thermal power. Unit: W with three decimal number, multiplied
by 1000 (mW).
• monthly_energy_consumption (array, read-only)
Monthly total energy consumption data. Array of objects with parameters:
◦ year (integer) - year of total energy consumption save
◦ month (integer) - month of total energy consumption save
◦ day (integer) - day of total energy consumption save
◦ value (integer) - total energy consumption for the time of saving the data. Unit: Wh.
Note: this is a total energy consumption value, not month energy consumption.
• energy_consumed_current_month (object, read-only, optional)
Current month energy consumption data. Can be nil when there is no monthly data
saved yet and no total energy consumption data read. Object with parameters:
◦ year (integer) - current year
...
• status (string, read-only)
• tags (string[])
• type (string, read-only) = "heat_meter"
• variant (string, read-only) = "generic"
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
• purpose (string)
...
• software_version (string, read-only)
Software name and version description.
776 SLINK devices: Analog input
Analog input
Analog input sensor representation. Measures value from analog input and sends it to
central unit.
Properties
...
• name (string)
• room_id (integer, read-only, optional)
777 SLINK devices: Analog input
• status (string, read-only)
• tags (string[])
• type (string, read-only) = "analog_input"
• variant (string, read-only) = "generic"
...
• endpoint (integer, read-only)
• software_version (string, read-only)
778 SLINK devices: Analog output
Analog output
Analog output representation. Set desired value to output in device.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
• labels (string[], read-only)
• messages (sequence, read-only)
779 SLINK devices: Analog output
• name (string)
• room_id (integer, read-only, optional)
• status (string, read-only)
• tags (string[])
...
780 SLINK devices: Blind controller
Blind controller
Controller opens and closes a roller shade, tilt blind or pergola.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
Note
Can be modified with values in allowed_lift_control_modes property.
781 SLINK devices: Blind controller
• allowed_lift_control_modes (table, read-only)
List of available lift control modes supported by the controller.
Possible values: current_detection , fixed_duration .
Required label: "percent_tilt_control"
...
Can be modified when: lift_control_mode is equal to fixed_duration .
Required label: "has_tilt_duration"
782 SLINK devices: Blind controller
• tilt_duration (number, optional)
Time required by motor to do full tilt cycle.
Unit: ms.
Required label: "has_tilt_cycle_distance"
...
HTML/Hex RGB representation of color when controller is active e.g. motor is working.
Example: #FFFF00
783 SLINK devices: Blind controller
Device properties (full spec)
• class (string, read-only) = "slink"
• color (string)
• icon (string)
...
Command is NOT available when window_covering_type is pergola .
Argument:
784 SLINK devices: Blind controller
opening percentage (number)
• up
Fully opens a blind.
Note
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
789 SLINK devices: CO2 sensor
SLINK device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
790 SLINK devices: Dimmer
Dimmer
Device that controlls light intensity of output LED.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
795 SLINK devices: Energy meter
Energy meter
Energy meter is a device which can track and count consumed energy (total so far and
daily) and sense voltage/current and active power.
Properties
...
• energy_consumed_total (number, read-only)
Total sum of energy used Unit: 1 Wh.
796 SLINK devices: Energy meter
Device properties (full spec)
• class (string, read-only) = "slink"
• color (string)
• icon (string)
...
power factor (cos 𝜑 = 1)) !
Arguments:
797 SLINK devices: Energy meter
packed arguments (table):
◦ expected voltage in mV
◦ expected current in mA
◦ expected active power in mW
...
798 SLINK devices: Flood sensor
Flood sensor
Device that detects water leak on flat surfaces.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
799 SLINK devices: Flood sensor
SLINK device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
...
800 SLINK devices: Humidity sensor
Humidity sensor
Humidity sensor. Measures humidity and sends measurement to central unit. Can be
assigned to virtual thermostat in web application.
Properties
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
801 SLINK devices: Humidity sensor
SLINK device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
802 SLINK devices: IAQ sensor
IAQ sensor
Battery powered Index of Air Quality sensor. Calculates Air Quality Index based on various
measures like CO2 or particles level and relative humidity. Sensors measure values only
every few minutes to save battery.
...
• endpoint (integer, read-only)
• software_version (string, read-only)
804 SLINK devices: Light sensor
Light sensor
Light sensor measures light illuminance in lux and sends measurement to central unit.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
805 SLINK devices: Light sensor
SLINK device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
806 SLINK devices: Motion sensor
Motion sensor
Motion sensor based on custom configuration checks whether motion was detected.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
Time since last motion detected. Value of -1 means there wasn’t any motion since last
system startup.
807 SLINK devices: Motion sensor
Note
The value will be 0 for each detected move, even if the previous one has not yet
finished.
Unit: seconds.
...
Commands
• enable
808 SLINK devices: Motion sensor
Enables motion detector.
• disable
Disables motion detection.
• add_time_since_motion_event
...
809 SLINK devices: Motion sensor
Enable a light for 5 minutes on motion detection
...
810 SLINK devices: Pressure sensor
Pressure sensor
Pressure sensor measures pressure and sends measurement to central unit.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
811 SLINK devices: Pressure sensor
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
SLINK device properties (full spec)
...
• endpoint (integer, read-only)
• software_version (string, read-only)
812 SLINK devices: Pulse width modulation
Pulse width modulation
Pulse width modulation device representation. Set desired signal in device output. User
can set desired frequency of signal and duty cycle i.e. the amount of time the digital signal
is in the “active” state relative to the period of the signal.
...
Argument:
duty_cycle (integer)
813 SLINK devices: Pulse width modulation
Examples
Set frequency and duty cycle of PWM
slink[3]:call("set_frequency", 3000)
slink[3]:call("set_duty_cycle", 40)
814 SLINK devices: RGB controller
RGB controller
Device that controlls color and light intensity of output LED.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
Warm white color temperature correction. Applies when led_strip_type set to
"rgbww" .
815 SLINK devices: RGB controller
• active_animation (number, read-only)
Active animation ID if animation was activated. Null value when no animation active.
Device properties (full spec)
• class (string, read-only) = "slink"
...
Turns on output.
Argument
816 SLINK devices: RGB controller
Optional: transition time in 0.1s (default 5 - 500ms) (number)
• turn_off
Turns off output.
Argument
...
• unit: Kelvin
◦ transition time (number):
817 SLINK devices: RGB controller
• minimum: 1 (100 ms)
• maximum: 6000 (10 minutes)
• unit: 100ms
• parameter is optional (500 ms default)
...
818 SLINK devices: RGB controller
Activate an animation by ID
local rgb = slink[79]
local animation_id = 2
rgb:call("activate_animation", { id = animation_id })
...
823 SLINK devices: Temperature regulator
Temperature regulator
Temperature regulator notifies when desired temperature is reached in room. Can be
assigned to virtual thermostat in web application.
Normally works in constant temperature mode only, but additional modes
...
maximum below target, will also change target value to minimum.
Unit: 0.1 °C
824 SLINK devices: Temperature regulator
• target_temperature_reached (boolean)
Controls device’s algorithm state indicator (available on some regulators). e.g. LED
Diode. May be controlled by external algorithms or devices such as Thermostat (when
thermostat is active, indicator will blink)
...
• color (string)
• icon (string)
825 SLINK devices: Temperature regulator
• id (integer, read-only)
• labels (string[], read-only)
• messages (sequence, read-only)
• name (string)
...
target temperature in 0.1°C (number)
• enable_constant_mode
826 SLINK devices: Temperature regulator
Calls Temperature Regulator to change target temperature mode to constant . When
regulator is already in constant mode, it will change mode target_temperature only.
Note
Cannot be executed when regulator is not associated with a thermostat.
...
827 SLINK devices: Temperature sensor
Temperature sensor
Temperature sensor. Measures temperature and sends measurement to central unit. Can
be assigned to virtual thermostat in web application as room or floor sensor.
Properties
...
• voice_assistant_device_type (string, read-only)
• purpose (string)
828 SLINK devices: Temperature sensor
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
SLINK device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
829 SLINK devices: Two state input sensor
Two state input sensor
Boolean input sensor checks input state and send it to central unit.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
• purpose (string)
• supervised_properties (array, read-only)
830 SLINK devices: Two state input sensor
• delayed_actions (array, read-only)
SLINK device properties (full spec)
• address (integer, read-only)
• endpoint (integer, read-only)
• software_version (string, read-only)
831 SLINK devices: Valve
Valve
Valve representation. Allows user to read and modify valve parameters.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
• endpoint (integer, read-only)
• software_version (string, read-only)
837 SLINK devices: Valve Analog PID
Valve Analog PID
0–10V valve with PID calculation representation. Allows user to read and modify valve
parameters.
Properties
...
Current valve water temperature.
Unit: 0.1 °C.
838 SLINK devices: Valve Analog PID
• proportional_gain (integer)
Proportional gain parameter for PID calculation.
Unit: 0.1 (no unit, parameter with one decimal number, multiplied by 10.)
Range: 0 – 100 (0.0 – 10.0)
...
• voice_assistant_device_type (string, read-only)
• purpose (string)
839 SLINK devices: Valve Analog PID
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
SLINK device properties (full spec)
• address (integer, read-only)
...
Unique (per physical device) identifier that help to distinguish same device types in one
physical device.
841 Alarm system: Satel — Alarm zone
Satel — Alarm zone
...
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
842 Alarm system: Satel — Alarm zone
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
...
843 Alarm system: Satel — Alarm zone
Examples
...
844 Alarm system: Satel — Two state input sensor
...
• purpose (string)
• supervised_properties (array, read-only)
845 Alarm system: Satel — Two state input sensor
• delayed_actions (array, read-only)
Alarm system properties (full spec)
• parent_id (string, read-only)
• software_version (string, read-only)
• sub_id (integer, read-only)
846 Alarm system: Satel — Two state output
...
• voice_assistant_device_type (string, read-only)
• purpose (string)
847 Alarm system: Satel — Two state output
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
Alarm system properties (full spec)
• parent_id (string, read-only)
...
848 Alarm system: Satel — Two state output
Examples
...
852 LoRa devices: Humidity sensor
Humidity sensor
Battery powered humidity sensor. Measures humidity and sends measurement to central
unit.
Sensors measure humidity only every few minutes to save battery. Can be assigned to
...
• purpose (string)
• supervised_properties (array, read-only)
853 LoRa devices: Humidity sensor
• delayed_actions (array, read-only)
LoRa device properties (full spec)
• signal (number, read-only)
• visible (boolean, read-only)
...
• sub_id (integer, read-only)
• battery (number, optional, read-only)
854 LoRa devices: Opening sensor
Opening sensor
Battery powered opening sensor. Checks whether window or door is open. Based on that
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
855 LoRa devices: Opening sensor
LoRa device properties (full spec)
• signal (number, read-only)
• visible (boolean, read-only)
• software_version (string, read-only)
...
• visible (boolean, read-only)
• voice_assistant_device_type (string, read-only)
860 LoRa devices: Temperature sensor
• purpose (string)
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
LoRa device properties (full spec)
...
• sub_id (integer, read-only)
• battery (number, optional, read-only)
861 LoRa devices: Two state input sensor
Two state input sensor
Boolean input sensor checks input state and send it to central unit.
Properties
Direct access to properties is not allowed. You can read or change values using getValue
...
• purpose (string)
• supervised_properties (array, read-only)
862 LoRa devices: Two state input sensor
• delayed_actions (array, read-only)
LoRa device properties (full spec)
• signal (number, read-only)
• visible (boolean, read-only)
...
864 System modules: WTP or SBus extenders
...
enable_diagnostic .
865 System modules: WTP or SBus extenders
Device properties (full spec)
...
• disable_diagnostic
866 System modules: WTP or SBus extenders
Turn off gathering diagnostic informations.
867 System modules: IR remote
IR remote
...
to detect it.
871 System modules: LoRa gateway
LoRa gateway
...
• uuid (string, read-only)
• enabled (boolean)
872 System modules: LoRa gateway
• software_version (string, read-only)
873 System modules: WTP or SBus Modbus transceiver
...
• messages (sequence, read-only)
• name (string)
874 System modules: WTP or SBus Modbus transceiver
• room_id (integer, read-only, optional)
• status (string, read-only)
• tags (string[])
...
• disable_diagnostic
Turn off gathering diagnostic informations.
875 System modules: Modbus transceiver
Modbus transceiver
...
876 System modules: Modbus transceiver
...
• enabled (boolean)
• software_version (string, read-only)
877 System modules: Modbus transceiver
Commands
Required label: "modbus_slave_support"
• enable_slave
...
Internal diagnostic data of extender. Has data if enabled by command
enable_diagnostic .
880 System modules: Modbus extender
• slave_mode (boolean, read-only)
Indicates if module is in slave mode. Can be changed only using commands
enable_slave or disable_slave .
Required label: "modbus_slave_support"
...
• supervised_properties (array, read-only)
• delayed_actions (array, read-only)
881 System modules: Modbus extender
System module properties (full spec)
• uuid (string, read-only)
• enabled (boolean)
• software_version (string, read-only)
...
• disable_diagnostic
Turn off gathering diagnostic informations.
882 System modules: Modbus extender
Examples
